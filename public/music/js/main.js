...
function loadMusicScript() {
  if (typeof localMusic === 'undefined' || !Array.isArray(localMusic) || localMusic.length === 0) {
    ...
    script.src = 'https://cdn.jsdelivr.net/gh/mcyzsx/ZSX-Music-MCY@main/js/Meting.js';
...
// 等待 ap 实例就绪后再绑定所有依赖 ap 的事件
function bindApEvents() {
  if (window.ap) { doBind(); return; }
  const t = setInterval(() => {
    if (window.ap) { clearInterval(t); doBind(); }
  }, 100);
